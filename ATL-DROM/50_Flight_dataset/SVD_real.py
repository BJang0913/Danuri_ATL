...
def main():
    ...
    n_modes = 40    # The number of training cases
    n_snaps = 40    # The number of training cases
...
def svd_scalar(n_modes,n_snaps,scalar,energyThreshold):
    ...
    snap_path = 'Training_output_real/14RD{0}.txt'
    ...
    if not os.path.isdir('POD_real'):
        os.mkdir('POD_real')

    for i in range(n_modes):
        if not os.path.isdir('POD_real/{0}'.format(i+1)):
            os.mkdir('POD_real/{0}'.format(i+1))
        
    

        pod_write = open('POD_real/{0}/{1}'.format(i+1,scalar),'w')
